...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/rando") {
      ...
      const home = await fetch("https://www.randoslorraine.org", {
        headers: { "User-Agent": "Mozilla/5.0" }
      }).then(r => r.text());

      // 2) Extraire automatiquement l’URL de la prochaine rando
      const nextUrl = extractNextRandoUrl(home);
      if (!nextUrl) {
        return new Response(JSON.stringify({ error: "Aucune rando trouvée" }), {
